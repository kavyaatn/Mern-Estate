        ...
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                ...
                <div className='flex flex-wrap gap-6'>
                    ...
                    <div className='flex flex-cols items-center gap-2'>
                        ...
                        <p>Regular Price
                        <span className='text-xs flex'>($ / month)</span> </p>
                    </div>
                    <div className='flex flex-cols items-center gap-2'>
                        <input type ='number' id='discountPrice' min='50' max="1000000" required className="p-3 border rounded-lg" onChange={handleChange} value={formData.discountPrice}/>
                        <p>Discounted Price
                        <span className='text-xs flex'>($ / month)</span> </p>
                    </div>
                </div>
            ...
            <div className='flex flex-col flex-1 gap-4'>
                ...
                <div className='flex gap-2'>
                    ...
                    <button type='button' onClick={handleFile} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80'>{uploading ? 'uploading...' : 'Upload'}</button>
                </div>
                <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) =>(
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            ...
                            <button type='button' onClick={() => handleClickRemove(index)}className='p-3 text-red-700 rounded-lg uppercase hover:opacity-95'>delete</button>
                        </div>
                    ))
                }
                <button className='p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-90'>{loading ? 'Creating...' : 'Create Listing'}</button>
            </div>
        </form>
    </main>
  )
